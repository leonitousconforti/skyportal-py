...
def fetch_tokens(client: httpx.Client, *, user_id: int | None = None) -> list[ApiToken]:
    ...
    return [ApiToken.model_validate(token) for token in unwrap(response)]


def fetch_token(client: httpx.Client, token_id: str) -> ApiToken:
    ...
    return ApiToken.model_validate(unwrap(response))
...
def update_token(
    ...
    *,
    ...
    user_id : int, optional
        User whose permissions the new ACLs are checked against; defaults
