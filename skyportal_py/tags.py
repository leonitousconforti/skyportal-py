...
def fetch_obj_tag_options(client: httpx.Client) -> list[ObjTagOption]:
    ...
    return [ObjTagOption.model_validate(item) for item in unwrap(response)]
...
def post_obj_tag_option(
    ...
    *,
    ...
    return ObjTagOption.model_validate(unwrap(response))
...
def fetch_obj_tags(
    ...
    *,
    ...
    return [ObjTag.model_validate(item) for item in unwrap(response)]
