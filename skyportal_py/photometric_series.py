...
def fetch_photometric_series(
    ...
    *,
    ...
    return PhotometricSeries.model_validate(unwrap(response))
...
def fetch_photometric_series_page(  # noqa: PLR0913 -- mirrors the endpoint's query parameters
    ...
    *,
    ...
    use_robust_mag_and_rms : bool, optional
        Filter on ``robust_mag``/``robust_rms`` instead of
    ...
    return PhotometricSeriesPage.model_validate(unwrap(response))
...
def post_photometric_series(
    ...
    return PhotometricSeriesPostResponse.model_validate(unwrap(response))
