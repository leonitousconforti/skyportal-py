...
class SharingServiceCoauthor(BaseModel):
    """A user credited as a coauthor of a sharing service's submissions."""
...
class SharingServiceGroupAutoPublisher(BaseModel):
    """A group member allowed to publish automatically through a service."""
...
class SharingServiceGroup(BaseModel):
    """A group's access to, and auto-publishing settings for, a service."""
...
class SharingService(BaseModel):
    """A service used to publish objects externally, e.g. to TNS or Hermes."""
    ...
    photometry_options: dict[str, Any] | None = None
...
class SharingServiceSubmission(BaseModel):
    """A request to publish one object through a sharing service."""
